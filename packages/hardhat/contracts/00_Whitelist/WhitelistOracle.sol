...
contract WhitelistOracle {
    ...
    function getPrice() public view returns (uint256) {
        ...
        for (uint256 i; i < oracles.length; i++) {
            ...
            if (timestamp != 0 && block.timestamp - timestamp <= STALE_DATA_WINDOW) {
                prices[count++] = price;
            }
        }
        if (count == 0) revert NoOraclesAvailable();
        uint256[] memory activePrices = new uint256[](count);
        for (uint256 i; i < count; i++) activePrices[i] = prices[i];
        activePrices.sort();
    ...
    function getActiveOracleNodes() public view returns (address[] memory) {
        address[] memory active = new address[](oracles.length);
        uint256 count;
        for (uint256 i; i < oracles.length; i++) {
            (, uint256 timestamp) = oracles[i].getPrice();
            if (timestamp != 0 && block.timestamp - timestamp <= STALE_DATA_WINDOW) {
                active[count++] = address(oracles[i]);
            }
        }
        address[] memory result = new address[](count);
        for (uint256 i; i < count; i++) result[i] = active[i];
        return result;
    }
}
