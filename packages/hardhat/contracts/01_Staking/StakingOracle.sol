...
contract StakingOracle {
    ...
    function registerNode(uint256 amount) public {
        ...
        if (!oracleToken.transferFrom(msg.sender, address(this), amount)) revert TransferFailed();
        nodes[msg.sender] = OracleNode(amount, 0, 0, 0, getCurrentBucketNumber(), true);
    ...
    function reportPrice(uint256 price) public onlyNode {
        ...
        uint256 bucket = getCurrentBucketNumber();
        OracleNode storage node = nodes[msg.sender];
        if (node.lastReportedBucket == bucket) revert AlreadyReportedInCurrentBucket();
        if (getEffectiveStake(msg.sender) < MINIMUM_STAKE) revert InsufficientStake();
        node.lastReportedBucket = bucket;
        node.reportCount++;
        blockBuckets[bucket].reporters.push(msg.sender);
        blockBuckets[bucket].prices.push(price);
        emit PriceReported(msg.sender, price, bucket);
    }
    ...
    function claimReward() public {
        OracleNode storage node = nodes[msg.sender];
        uint256 unclaimed = node.reportCount - node.claimedReportCount;
        if (unclaimed == 0) revert NoRewardsAvailable();
        node.claimedReportCount = node.reportCount;
        uint256 reward = unclaimed * REWARD_PER_REPORT;
    ...
    function recordBucketMedian(uint256 bucketNumber) public {
        ...
        uint256 requiredReports = (nodeAddresses.length * 2 + 2) / 3;
        if (bucket.prices.length < requiredReports || bucket.prices.length == 0) revert MedianNotRecorded();
    ...
    function slashNode(
        ...
        if (reportIndex >= bucket.reporters.length || bucket.reporters[reportIndex] != nodeToSlash) {
            revert NodeNotAtGivenIndex();
        }
        if (!_checkPriceDeviated(bucket.prices[reportIndex], bucket.medianPrice)) revert NotDeviated();

        bucket.slashedOffenses[nodeToSlash] = true;
        OracleNode storage node = nodes[nodeToSlash];
        uint256 penalty = node.stakedAmount < MISREPORT_PENALTY ? node.stakedAmount : MISREPORT_PENALTY;
        node.stakedAmount -= penalty;
        uint256 reward = (penalty * SLASHER_REWARD_PERCENTAGE) / 100;
        if (reward != 0 && !oracleToken.transfer(msg.sender, reward)) revert TransferFailed();
        emit NodeSlashed(nodeToSlash, penalty);
        if (node.stakedAmount == 0) _removeNode(nodeToSlash, nodeAddressesIndex);
    }
    ...
    function exitNode(uint256 index) public onlyNode {
        OracleNode storage node = nodes[msg.sender];
        uint256 referenceBucket = node.lastReportedBucket == 0 ? node.firstBucket : node.lastReportedBucket;
        if (getCurrentBucketNumber() < referenceBucket + WAITING_PERIOD) revert WaitingPeriodNotOver();
        uint256 amount = getEffectiveStake(msg.sender);
        _removeNode(msg.sender, index);
        if (amount != 0 && !oracleToken.transfer(msg.sender, amount)) revert TransferFailed();
        emit NodeExited(msg.sender, amount);
    }
     ...
     */
    function getNodeAddresses() public view returns (address[] memory) { return nodeAddresses; }
    ...
    function getLatestPrice() public view returns (uint256) {
        uint256 current = getCurrentBucketNumber();
        if (current <= 1 || blockBuckets[current - 1].medianPrice == 0) revert MedianNotRecorded();
        return blockBuckets[current - 1].medianPrice;
    }
    ...
    function getPastPrice(uint256 bucketNumber) public view returns (uint256) {
        uint256 median = blockBuckets[bucketNumber].medianPrice;
        if (median == 0) revert MedianNotRecorded();
        return median;
    }
    ...
    function getSlashedStatus(
        ...
        BlockBucket storage bucket = blockBuckets[bucketNumber];
        for (uint256 i; i < bucket.reporters.length; i++) {
            if (bucket.reporters[i] == nodeAddress) return (bucket.prices[i], bucket.slashedOffenses[nodeAddress]);
        }
        return (0, bucket.slashedOffenses[nodeAddress]);
    }
    ...
    function getEffectiveStake(address nodeAddress) public view returns (uint256) {
        ...
        uint256 elapsed = getCurrentBucketNumber() - node.firstBucket;
        uint256 missed = elapsed > node.reportCount ? elapsed - node.reportCount : 0;
        uint256 penalty = missed * INACTIVITY_PENALTY;
        return penalty >= node.stakedAmount ? 0 : node.stakedAmount - penalty;
    }
    ...
    function getOutlierNodes(uint256 bucketNumber) public view returns (address[] memory) {
        ...
        uint256 count;
        for (uint256 i; i < bucket.reporters.length; i++) {
            if (_checkPriceDeviated(bucket.prices[i], bucket.medianPrice)) temp[count++] = bucket.reporters[i];
        }
    ...
    function _removeNode(address nodeAddress, uint256 index) internal {
        ...
        nodes[nodeAddress].stakedAmount = 0;
    }
    ...
    function _checkPriceDeviated(uint256 reportedPrice, uint256 medianPrice) internal pure returns (bool) {
        uint256 difference = reportedPrice > medianPrice ? reportedPrice - medianPrice : medianPrice - reportedPrice;
        return difference * 10_000 > medianPrice * MAX_DEVIATION_BPS;
    }
}
