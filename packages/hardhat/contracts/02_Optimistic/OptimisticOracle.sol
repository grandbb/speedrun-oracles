...
contract OptimisticOracle {
    ...
    function assertEvent(
        ...
        uint256 actualStart = startTime == 0 ? block.timestamp : startTime;
        uint256 actualEnd = endTime == 0 ? actualStart + MINIMUM_ASSERTION_WINDOW : endTime;
        if (actualEnd < actualStart + MINIMUM_ASSERTION_WINDOW) revert InvalidTime();
    ...
    function claimUndisputedReward(uint256 assertionId) external {
        ...
        uint256 amount = assertion.reward + assertion.bond;
        (bool success,) = assertion.proposer.call{value: amount}("");
        if (!success) revert TransferFailed();
        emit RewardClaimed(assertionId, assertion.proposer, amount);
    }
    ...
    function claimDisputedReward(uint256 assertionId) external {
        ...
        assertion.claimed = true;
        uint256 winnerAmount = assertion.reward + assertion.bond;
        (bool feeSuccess,) = decider.call{value: assertion.bond}("");
        if (!feeSuccess) revert TransferFailed();
        (bool winnerSuccess,) = assertion.winner.call{value: winnerAmount}("");
        if (!winnerSuccess) revert TransferFailed();
        emit RewardClaimed(assertionId, assertion.winner, winnerAmount);
    }
    ...
    function claimRefund(uint256 assertionId) external {
        ...
        if (block.timestamp <= assertion.endTime) revert InvalidTime();
        assertion.claimed = true;
        (bool success,) = assertion.asserter.call{value: assertion.reward}("");
        if (!success) revert TransferFailed();
    ...
    function getState(uint256 assertionId) external view returns (State) {
        ...
        if (assertion.proposer != address(0)) {
            return block.timestamp > assertion.endTime ? State.Settled : State.Proposed;
        }
        return block.timestamp > assertion.endTime ? State.Expired : State.Asserted;
    }
